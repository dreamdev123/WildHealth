...
public class SendDorothyCommsOnClaimSubmittedEvent : INotificationHandler<ClaimSubmittedEvent>
{
    private readonly IMediator _mediator;
    private readonly IOptions<PracticeOptions> _options;

    public SendDorothyCommsOnClaimSubmittedEvent(IMediator mediator, IOptions<PracticeOptions> options)
    {
        _mediator = mediator;
        _options = options;
    }
    ...
    public async Task Handle(ClaimSubmittedEvent notification, CancellationToken cancellationToken)
    {
        ...
        {
            var command = new SendDorothyClaimSubmittedCommsCommand(notification.ClaimId);
