...
{
    public class StartInternalConversationCommandHandler : IRequestHandler<StartInternalConversationCommand, Conversation>
    {
        ...
        public StartInternalConversationCommandHandler(
        ...
        public async Task<Conversation> Handle(StartInternalConversationCommand command, CancellationToken cancellationToken)
        {
            ...
            var employees = await _employeesService.GetByIdsAsync(command.EmployeeIds, EmployeeSpecifications.ActiveWithUser);

            var conversation = ConversationDomain.CreateConversation(
                subject: $"Internal conversation about patient: {patient.User.GetFullname()}",
